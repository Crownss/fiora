...
impl super::UserDataStore {
    ...
    pub async fn list_user(&self, mut req: Req) -> Res<(Vec<UserEntity>, i64)> {
        ...
        let query =
            "select id, bookid as borrowed_book_id, firstname as first_name, lastname as last_name, email, username, password, createdtime, updatedtime from users order by createdtime desc limit $1 offset $2";
    ...
    pub async fn get_user_by(&self, req: ReqFilter) -> Res<UserEntity> {
        let mut query = String::from("select id, bookid as borrowed_book_id, firstname as first_name, lastname as last_name, email, username, createdtime, updatedtime from users where email!=$1 and firstname!=$2 and lastname!=$3 and username!=$4 and bookid!=$5");
        if req.email != "" {
